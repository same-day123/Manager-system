...
- **一律「只补不重建」**：空库重建会把资产 **32 → 25 台**（丢 7 台早期 `LAB-CHEM-*`），与已交付的 T6 截图（资产台账 32 行）对不上 → 截图与现场不一致比数字矛盾更糟。真实库 `education_system` 截至 2026-09-17 **仍未执行**校准（执行会作废 T6-01 首页看板截图，需跨角色返工），等阿辉授权（待确认表 **Q-19**）。
