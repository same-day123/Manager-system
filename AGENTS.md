...
| **T1** | **测试负责** | 单元测试 7 → 29 个 | P0 | T0 | 测试文档「单元测试 ≥10」 | 🔄 | **2026-09-17 测试负责认领开工**（不依赖 T0，Mockito 单测不连库） |
...
| **D-17** | **`README.md`「已知待办」最后一条仍写「未初始化 Git 仓库（当前无 `.git`）」，而 D-09 已于 2026-09-17 解除**（`git init -b main` + `origin` 已挂，**且已成功推送**） | 文档漂移 —— **第 7.1 节 C4「README → 事实」检查不通过** | README 是助教与新组员读的第一份材料，此处自相矛盾会让人以为仓库仍未纳入版本控制（进而怀疑 T3 能否触发流水线） | 删掉或改写该行。`README.md` **不在「文档负责」可写目录**（见 5.1 节硬边界），故**只登记、不代改** → 交 **「代码负责」或「产品经理」** 收口 | ❌ 未修 |
