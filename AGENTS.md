...
**演示账号**：`labadmin` / `asset01` / `repair01` / `room01` / `student1` / `viewer01`，密码统一 `admin123`。
...
| **测试负责** | 职责六 | **T1、T2** | 交付物 3 的**全部数字与截图** | `mvnx.ps1 -o -B clean test` 的 `Tests run` 达标且截图留痕 | `RuoYi-Vue-fast/src/test/**`、`RuoYi-Vue-fast/pom.xml`（**仅限 test-scope 依赖**）、`AI留痕/T1-*.md`、`AI留痕/T2-*.md`、`AI留痕/测试素材交付.md`（**不改 `src/main`**） |
| **文档负责** | 职责五 | **D1**（五份 PDF） | **交付物 1~5，合计 70 分** | 五份页数命中各自区间；每份开头有 AI 使用声明；查重自检 | `docs/大作业/文档/**`、`AI留痕/D1-*.md`（**不碰代码、不动第 6 / 7 节台账**） |
| **UI设计** | 职责二 | **T6** | 交付物 6「核心功能界面」截图；演示观感 | T6 卡第 6 节 6 条 grep；`cd RuoYi-Vue3` 后 `vite build` 成功 | `RuoYi-Vue3/src/views/laboratory/**`、`assets/styles/**`、`AI留痕/T6-*.md`、`AI留痕/截图/**`（**不改首页 `views/index.vue` 视觉基线**） |
...
> **各角色 `AI留痕/` 写权限约定（2026-09-18 补，收编原文字缺口）**：每个角色可在 `docs/大作业/AI留痕/` 下写**自己卡号开头的留痕文件**（如「测试负责」写 `T1-*`/`T2-*`、「UI设计」写 `T6-*`、「代码负责」写 `T0/T3/T4/T5-*`、「PPT和设计」写 `D2-*`、「文档负责」写 `D1-*`）。**不得改别人的卡号留痕**。
> **任务卡回写约定（同批收编）**：执行人可在**自己认领的任务卡**末尾追加「验收结果」小节回写实测值；**不改任务卡的「交付内容 / 边界」主体**，不新增/删除卡。
> 这两条是对上表「可写目录」列的补充，**不是放宽源码边界**——`src/**` 与 `.github/`、`deploy/`、`sql/` 仍严格按角色行执行。
...
| 2026-09-18 | **文档负责（AI）** | **第 3、4 份初稿 + 导出 PDF，D1 五份交付物全部出稿**：**第 3 份《测试计划与用例文档》**（**7 页，PASS**，区间 6–10）——硬指标逐项命中：金字塔比例（单测 33 / 集成 16 / 手工 2 条 + 每层理由 + E2E 低占比主动说明未引 Selenium）／单元测试 **33**（5 类）／集成 **16**（3 类，含看板 7 用例）／等价类有效无效各一组／边界值 **7 组**／**两条如实说明已写进第 9 章正文**（覆盖率 % 出不来 → 用例↔BR 矩阵 + 断言 177 / 交互校验 27，不编百分比；看板 8 条查询已覆盖、生产 mapper XML 一字未改）／2 处测试截图留 `[待填]` 待人工截取 PNG。**第 4 份《CI/CD 部署方案》**（**5 页，PASS**，区间 5–8）——硬指标逐项命中：三环境差异表（8 维度）/ 全链路流水线图 + 四 job 表（引用仓库 `ci.yml`，不整份重贴）/ 部署策略**金丝雀**（排除法理由：滚动缺 K8s、蓝绿资源翻倍+Redis 会话掉线）/ Dockerfile 加分项。**两条红线已遵守**：未等 `docker compose ps` 截图（§6.2「已验证/未验证」分开，94 条静态断言 + 23 条环境变量绑定实证作替代证据）；未照抄 T4 卡错误示例（§6.3 单列卡两处缺口及修正）。**全部数字取自真实构建日志与仓库文件，无一估算**。待办（统一）：补封面 `[待填]` + 人工改写防查重 + 03 补 2 张截图后重新导出 | `docs/大作业/文档/03-测试计划与用例文档.md`、`04-CI_CD部署方案.md` + `pdf/` 同名 PDF；`AI留痕/D1-2026-09-17.md` 第 15、16 节 |
| 2026-09-17 | **代码负责（AI）** | **D-24 / D-25 演示数据校准（脚本已就绪并双路径验证，真实库待授权）**：① **核现状推翻了两条偏移的原始根因** —— 真实库实测 **5 台** `status='2'` 资产**全部没有工单**（不止 D-24 记的 3 台）；D-25 也不是"种子写死日期"（`[5/7]` 早就是相对日期），而是真实库躺的是脚本改写前的旧行、且报修单有 `repair_code` 幂等保护 → **只插不更新，日期永远停在第一次插入那天**；② **发现种子自身不自洽**——`[4/7]` 把 `LAB-DEMO-2026-009`/`024` 置「维修中」却没给它们建任何工单，所以**光重建也治不好**（影子库实测：维修中 2 台 / 在途工单 0 张）；③ **否决"清空重建"（Q-17 A）**：空库重建会把资产 **32 → 25 台**（丢掉 7 台早期 `LAB-CHEM-*`），与已交付的 T6 截图（32 行）不一致；④ **否决"改看板聚合"（Q-18 A）**：与需求基线「近七日趋势」冲突，且会打破 T2 看板集成测试 DB-01~DB-07 的窗口边界断言（**属测试负责的模块，不该由我越界改**）；⑤ 落地：`[5/7]` 补 **2 张在途工单**（1 待审核 + 1 维修中，含 `提交报修` 与 `状态流转` 履历，文案逐字对齐 `LabRepairEvent`；`repair_user` 走 **left join**，否则待审核单整行插不进去）＋ 新增 **`sql/laboratory_demo_refresh.sql`**（幂等：先补履历后改状态，把「无在途工单的维修中资产」归位；再把 6 张演示单日期平移回近 7 天）；⑥ 顺带改正 `[5/7]` 一条履历的引号（直角引号「」→ 全角 “”，与生产代码/真实库一致）；⑦ **验证**：影子库**双路径**——克隆真实库路径修复前 **3/12**、修复后 **12/12**、幂等复跑不变、资产规模 **32 台不回退**；空库重建路径 8 个脚本零报错、12/12。**真实库 `education_system` 全程未改动**，等授权。断言工具 `demo_refresh_verify.sql`（12 条，含"维修中资产数 = 有在途工单的资产数"这条关键不变式），完整证据 `.workbuddy/logs/demo_refresh_verify.txt`。⑧ **阿辉 2026-09-18 00:0x 授权后立即执行**：先 `mysqldump` 备份（`.workbuddy/logs/db-backup-education_system-20260918.sql`，113K / 36 表），再依次跑 `[5/7]` + `laboratory_demo_refresh.sql` —— 结果 **维修中资产 5→2、在途工单 0→2、待审核 1、近七日 5 天有柱子（09-16 ¥120）、12/12 断言全绿、资产 32 台不变、幂等复跑不变**；**副作用已按 6.2 交件台通知「前端UI负责」重拍 `T6-01` 首页看板截图** | `RuoYi-Vue-fast/sql/laboratory_user_seed.sql`、`RuoYi-Vue-fast/sql/laboratory_demo_refresh.sql`（新）、`README.md`、`AGENTS.md`；`.workbuddy/tools/demo_refresh_verify.sql`；`AI留痕/巡检-演示数据校准-2026-09-17.md` |
...
| **D2** | **PPT和设计** | 答辩 PPT 设计与视觉物料（≤15 页） | P1 | D1（内容） | 答辩表现 25 分；交付物 6 | 🔄 | **2026-09-17 三轮推进**：① 产出 `答辩PPT/slide-outline.md`（14 页预算）、`答辩PPT/视觉规范.md`、画布 4 版；② **阿辉批准定稿封面 / 第 3 页架构 / 第 4 页状态机**（色值全量回查通过、骨架坐标逐项一致；第 3 页页脚补全 4 处 SOLID，素材引自 `文档/02` §4.1）；③ **阿辉下「先填与代码状态无关的页」，本轮一次出 6 页**（02 背景与三痛点 / 05 设计取舍 / 06 AI 协作双向纠错 / 10 权限口径 / 12 三条容忍偏差 / 14 分工与 AI 声明），**累计 9 页 / 14 页**；结构校验**零溢出零重叠**，6 张导出图落 `答辩PPT/导出预览/`。留痕：`AI留痕/D2-2026-09-17.md`（第 14~22 节 = 第 3 轮）。<br>**2026-09-18 第 4 轮：14/14 页全部出稿 + 实体文件已产出**：④ 一次补齐 **07 测试金字塔 / 08 交付链路 / 09 演示导引 / 11 数据模型 / 13 规模指标**（素材：T3 实测 `PASS 4 / SKIPPED 2 / FAIL 0`、测试数按**阿辉批示落 49**、73 条接口 0 失败、D-28 替代证据）；⑤ 12 页扩为**五条容忍偏差**（加入 D-26 / D-28）、14 页补六角色分工清单；⑥ **实体文件**：`答辩PPT-v1.pptx`（14 页）+ `答辩PPT-v1.pdf`（14 页，逐页 PNG→Chrome CDP 合成；`export_nodes pdf` 会拍平成 1 页，弃用）+ 14 张导出预览图；⑦ **发现并修复 pptx 导出缺陷**——导出器剥掉文本首尾 CJK/全角字符串（297 个 run 缺段、10/14 页受影响；画布与 PNG/PDF 不受影响），以画布为真值本地回填 → **交付以 `答辩PPT-v2.pptx` 为准**（逐页校验 0 缺失；v1 仅作缺陷标本留档）。留痕：同文件第 23~28 节 = 第 4 轮。<br>**仅剩两项外部输入**：① 第 9 页三个截图位（占位中；T6-01 已重拍，待替换后重导出）；② 分工认领 Q-08（第 14 页占位，只换文字不动结构）。~~测试数字 37/49~~ ✅ 已定 49。D5 海报：阿辉已定**先不做**（Q-07） |
| **D5** | **PPT和设计** | 海报与对外视觉物料 | P2 | 阿辉定用途 | 演示观感；创新分（5 分）加分尝试 | ⚠️ | 等阿辉确认做哪几张、用在哪 |
| **D4** | **产品经理** | 共享上下文对齐 | — | — | 四角色协同同步 | ✅ | 2026-09-17 |
| **D1** | **文档负责** | 五份文档撰写（交付物 1~5） | P1 | T0~T5 | **文档质量 70 分** | ✅ | 卡：`任务卡/D1-五份文档撰写.md`。**2026-09-18 5/5 份 PDF 全部出稿**：01 需求 8 页 / 02 架构 13 页 / 03 测试 / 04 CI/CD / 05 AI 报告（`文档/pdf/01~05.pdf` 全部实存；03/04 为本轮新产出）。**收尾待办**：① 封面空位（编写人/审核人/日期/学院/其余 3 人学号）等阿辉提供后回填；② 03/04 若刚出稿需补验页数区间 + AI 使用声明 |
| **D3** | **代码负责** | 源码 ZIP 打包 | P0 | 全部 + D1 | 交付物 7 | 🟡 | **2026-09-18 脚本与清单已备好，9-20 一键出包**：`.workbuddy/tools/d3_package.py`（用 `git archive` 只导已跟踪文件，天然排除 target/dist/node_modules/logs + 23 个巡检临时脚本，**不用真删任何东西**）+ `.workbuddy/tools/d3_package_清单.md`（命名/内容边界/操作步骤）。**5 份 PDF 已实测齐全**（03/04 是文档负责新产出、尚未 commit），脚本**从工作区读定稿版覆盖进 ZIP**（避开 git 历史旧版）。dry-run + 真实打包均验证通过（798 文件 / 8.58 MB / PDF 恰好 5 份零重复） |
...
> 「代码负责」**四张卡（T0 / T3 / T4 / T5）已全部完成**，**只剩 `D3` 源码 ZIP 打包**（T-2 天执行，需「文档负责」交完 5 份 PDF 后开工）。**2026-09-18 更新**：D3 打包脚本与清单已备好，**5 份 PDF 已实测齐全**（03 测试 / 04 CI/CD 也已出），9-20 直接 `python .workbuddy/tools/d3_package.py` 出包即可。
...
| 日期 | 从 → 到 | 交了什么 | 位置 | 下游约束 |
| --- | --- | --- | --- | --- |
| **2026-09-18 00:1x** | **代码负责 → 前端UI负责 / PPT和设计** | **⚠️ 真实库演示数据已校准（D-24 / D-25 已修），`T6-01-首页运维看板` 截图当场过期，需重拍**：校准后首页数字发生变化 —— 维修中资产 **5 → 2**、待审核报修 **0 → 1**、已完成报修 **3**；「近七日报修趋势」由**空线变成 5 天有柱子**（09-13/14/15/16/17）、「近七日维修费用」**09-16 出现 ¥120**（原来也是空线）。**另新增可演示路径**：报修列表多出 2 张**在途**工单（`BX202607020005` **待审核**、`BX202607020006` **维修中**）→ 现场可完整走「审核」与「推进到已完成」，报修处理时间线也有内容了 | 主证据 `AI留痕/巡检-演示数据校准-2026-09-17.md` 第 5 / 6 节；证据日志 `.workbuddy/logs/demo_refresh_verify.txt`；真实库备份 `.workbuddy/logs/db-backup-education_system-20260918.sql` | **① 重拍 `T6-01`（1920×1080、`admin` 取景）前先确认后端已重启读到新数据**（数据层已变，无需重启即生效，但前端页面要刷新；若后端在跑，直接刷新即可）。**② 其余 3 张（资产台账 / 资产履历 / 报修时间线）不受影响**，资产仍是 **32 台**、总数不变。**③ 答辩 PPT 里原先「避开两张空图」的排版建议作废** —— 现在 4 张图都有数据，可整图使用。**④ 答辩前 1 天需再跑一次 `laboratory_demo_refresh.sql`**（日期会自然老化，一次平移约维持 7 天），跑完同样要刷新截图，**建议截图定稿与最终校准排在同一天** |
| **2026-09-18 18:1x** | **UI设计 → PPT和设计** | **✅ `T6-01-首页运维看板.png` 已重拍完成（旧图同路名覆盖作废）**：2026-09-18 18:12、`admin`（口令 `admin123`，Q-15 已定）取景、1920×1080、无弹窗遮挡。**画面与校准后真实库逐一核对一致** —— 指标卡 **资产总数 32 / 维修中 2 / 待审核 1 / 已完成 3**；「近七日报修趋势」**5 天有数据**（09-17 峰值 2、共 6 单）；「近七日维修费用」**09-16 ¥120 峰值**、右上角汇总 **¥120.00**；4 张 canvas 全渲染、无降级提示、无横向滚动。**另实测发现：看板趋势图坐标轴随系统日期滚动**（新图右端是 09-18）→ **9-19 冻结当天画面依然真实，无需再重拍**；但**答辩前若又跑了 `laboratory_demo_refresh.sql` 校准**，同一天刷新页面重截一张即可（脚本 `.workbuddy/tools/reshoot_t6_01.mjs` 一键重拍） | `docs/大作业/AI留痕/截图/T6-核心功能界面/T6-01-首页运维看板.png`（109.5 KB）；重拍日志 `.workbuddy/logs/reshoot_t6_01.log`；画面数字接口核对 `.workbuddy/logs/dashboard_numbers.txt` | **① 本行取代 2026-09-17 22:0x 行中「T6-01 两张空图、排版避开」的警告**（该警告已不适用，4 张图现在全部有数据、可整图排版）。**② 其余 3 张（T6-02/03/04）沿用 9-17 版**，资产 32 台未变。**③ 9-19 冻结前务必把新 T6-01 换进 PPT**（旧空图版本不要再用） |
| 2026-09-17 22:0x | **UI设计 → PPT和设计** | **D2 第 3 类「核心功能界面」4 张截图**（1920×1080、统一 `admin` 取景、无遮挡、可直接排版）：`T6-01-首页运维看板` / `T6-02-资产台账` / `T6-03-资产履历时间线` / `T6-04-报修处理时间线`。另附 5 张验收证据图（V-1/V-2/V-2b/V-4/报修列表/P2）+ 1 张二维码弹窗图（P4） | `docs/大作业/AI留痕/截图/T6-核心功能界面/`；交件单明细见 `AI留痕/T6-2026-09-17.md` **第 9.1 节**；演示路径巡检结论见 `AI留痕/巡检-UI演示路径-2026-09-17.md` | **⚠️ 必须在 2026-09-19（D-3）冻结点前定稿用进 PPT**；9-19 后再改 UI 这批图作废。**⚠️ T6-01（首页看板）里「近七日报修趋势」「近七日维修费用」两张图是空的（D-25）**——排版时建议只取左上 4 指标卡 + 右上故障等级分布 + 左下实验室维修次数，**避开两张空图**；若 D-25 修复后可重拍。**【2026-09-18 18:1x 更新（UI设计自注）：T6-01 已重拍、两张空图已消失，本行空图警告作废，以交件台最新一行为准】** |
...
| **D-20** | **演示账号的密码与全部文档不符**：`laboratory_user_seed.sql:23` 写入的 `@default_password` 是 `$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2`（**对应 `admin123`**，若依原生种子的哈希），但**同一文件第 9~21 行的注释**、`README.md` 演示账号表、`AGENTS.md` 第 1 节都写「密码统一 `123456`」 | 文档 ↔ 事实漂移（复现性） | **答辩现场按文档登录会当场失败**：实测 `labadmin` + `123456` → `{"msg":"用户不存在/密码错误"}`；`labadmin` + `admin123` → 登录成功。老师按 README 试也登不进 | **已修（2026-09-18 阿辉定「走 A：改文档为 admin123」）**：三处文本已统一改为 `admin123` —— ① `laboratory_user_seed.sql` 第 9 行注释 + 第 16~21 行账号清单；② `AGENTS.md` 第 1 节演示账号表；③ `README.md` 第 231 行。**不改脚本哈希、不改真实库**（零风险）。注意：`ry_20260417.sql:563` 的 `sys.user.initPassword='123456'` 是**另一个概念**（若依「新建用户初始密码」配置项），与演示账号无关，**保持不动** | ✅ 已修 |
...
| **D-24** | **真实库资产状态与 `laboratory_demo_seed.sql` 不一致，导致首页看板出现「5 台维修中资产 / 0 条待处理工单」的自相矛盾**。`laboratory_demo_seed.sql` 里只设 2 台「维修中」（`LAB-DEMO-2026-009`、`LAB-DEMO-2026-024`，均备注"报修中资产演示"），但真实库有 **5 台** `status='2'`（额外多出 2007 `LAB-CHEM-2026-001`、2030 `LAB-DEMO-2026-023`、2032 `LAB-DEMO-2026-025`）；多出的 3 台在 `lab_asset_record` 中显示为 **2026-07-03 17:01 由 admin 直接做「资产状态变更」**，并非通过报修流程生成。此外 `LAB-CHEM-2026-001` 在**任何 SQL 脚本里都查不到**，来源不明 | 数据质量 / 交付物一致性（种子脚本↔真实库） | ① 首页看板最醒目的 4 个指标卡里，**「维修中资产 5」与「待审核报修 0」并置**，答辩开场就会被问「5 台在修为什么没有工单」；② `laboratory_demo_seed.sql` 是 `where not exists` 插入 → 只插不更新，**按脚本重建的演示库与当前真实库状态不一致**，交付物 7 的可复现性受损 | 修复建议（二选一）：A) 9-18 清空业务表后重跑 `[N/7]` 脚本，让真实库与种子脚本一致；B) 写一条幂等 `UPDATE` 脚本把多出的 3 台改回 `status='0'`（风险低、保留既有房间/资产编号）。**归属：代码负责 / T0 卡**。AI留痕/巡检报告见 `AI留痕/巡检-UI演示路径-2026-09-17.md` 第 4 节<br>**⚠️ 2026-09-17（代码负责）核现状后修正了根因并备好修复**：原判断「真实库比种子多 3 台」只对了一半 —— ① 真实库实测 **5 台** `status='2'`，且**这 5 台一张工单都没有**（不止 3 台孤儿）；② 更关键的是**种子脚本自身就不自洽**：`[4/7]` 把 `LAB-DEMO-2026-009`/`024` 置「维修中」却没给它们建任何工单 → **光重建也治不好**（影子库重建实测：维修中 2 台、在途工单 0 张）；③ 空库重建会把资产 **32 台掉到 25 台**（丢掉 7 台早期 `LAB-CHEM-*`），与已交付的 T6 截图（32 行）不一致 → **否决 A 方案**。修法：`[5/7]` 补 2 张在途工单（1 张待审核可现场演示审核 + 1 张可现场推进到已完成）＋新增 `sql/laboratory_demo_refresh.sql`（幂等：纠偏孤儿 + 日期平移）。**两条修复路径均在影子库验证，12/12 断言全绿、幂等复跑不变**，真实库规模保持 32 台不回退 | 🟡→✅ **2026-09-18 00:0x 经阿辉授权后已在真实库执行完毕**（先 `mysqldump` 备份至 `.workbuddy/logs/db-backup-education_system-20260918.sql`，再依次跑 `[5/7]` + `laboratory_demo_refresh.sql`）：维修中资产 **5 → 2**、在途工单 **0 → 2**、待审核报修 **1**、近七日趋势 **5 天有柱子（09-16 含 ¥120 成本）**、断言 **12/12 全绿**、资产规模 **32 台不变**、幂等复跑不变。**⚠️ 副作用：T6-01「首页运维看板」截图当场变成过期证据，必须重拍**（见 6.2 交件台第 1 行处置）。**✅ 2026-09-18 18:1x（UI设计 第三轮）已重拍完成**：新图画面 32/2/1/3 + 两图 5 天有柱（09-16 ¥120），与本校准结果逐一核对一致；旧图同路名覆盖作废，见 6.2 交件台「2026-09-18 18:1x UI设计」行 |
| **D-25** | **首页看板「近七日报修趋势」「近七日维修费用」两张 ECharts 图为空线（0 单 / ¥0.00）**。直接证据：T6-01 首页运维看板截图左下、右下两图全是 0 | 演示数据 / 可视化 | 首页是答辩开场画面，4 张图里有 2 张为空，演示观感严重下降 | 根因：`LabDashboardMapper` 用 `date_sub()` 过滤最近 7 天，而所有演示工单日期都是 2026-07-02/03（种子写死），当前 2026-09-17 全部被过滤掉。**修复归属：代码负责 / T0 卡**。建议：A) 后端改为「全部时间」/「按创建月份」聚合；B) 或把种子数据 `create_time` 改成相对演示日期前 7 天内。证据见 `AI留痕/巡检-UI演示路径-2026-09-17.md` 第 4 节与 dashboard_numbers.txt<br>**⚠️ 2026-09-17（代码负责）根因修正 —— 不是「种子写死日期」**：`[5/7]` 早已用相对日期（`date_sub(sysdate(), interval N day)`），真实库里躺的是 **2026-06-27 ~ 07-01 的旧行**（脚本改写之前插进去的）。而报修单有 `repair_code` 幂等保护（`where not exists`）→ **只插不更新，日期永远停在第一次插入那天**。所以脚本没错，真正的性质是「相对日期会自然老化」。**同时这也否决了 A 方案**：改成「全部时间」聚合与需求基线「近七日趋势」冲突，还会打破 T2 看板集成测试 DB-01~DB-07 的窗口边界断言（那是测试负责的模块）；数据侧校准零风险、不动一行生产代码。修法：新增 `sql/laboratory_demo_refresh.sql` 把演示单日期重新平移回近 7 天，**可重复执行，答辩前跑一次即可**（一次平移约可维持 7 天）。另顺带改正 `[5/7]` 里一条履历的引号（原写直角引号「」、生产代码与真实库都是全角 “”，脚本↔运行时文案不一致） | 🟡→✅ **2026-09-18 00:0x 经阿辉授权后已在真实库执行完毕**（先 `mysqldump` 备份至 `.workbuddy/logs/db-backup-education_system-20260918.sql`，再依次跑 `[5/7]` + `laboratory_demo_refresh.sql`）：维修中资产 **5 → 2**、在途工单 **0 → 2**、待审核报修 **1**、近七日趋势 **5 天有柱子（09-16 含 ¥120 成本）**、断言 **12/12 全绿**、资产规模 **32 台不变**、幂等复跑不变。**⚠️ 副作用：T6-01「首页运维看板」截图当场变成过期证据，必须重拍**（见 6.2 交件台第 1 行处置）。**✅ 2026-09-18 18:1x（UI设计 第三轮）已重拍完成**：新图画面 32/2/1/3 + 两图 5 天有柱（09-16 ¥120），与本校准结果逐一核对一致；旧图同路名覆盖作废，见 6.2 交件台「2026-09-18 18:1x UI设计」行 |
...
| **D-29** | **5.1 节「可写目录」边界表存在 3 处文字性缺口**（改动均有任务卡依据、台账公开记载，**非行为违规、非静默越界**，性质同「共享基建目录」收编前）：① `RuoYi-Vue-fast/pom.xml` 被「测试负责」加了 `h2` test 依赖（T2 需要），但 pom.xml 不在 `src/test/**` 边界内、也不在任何角色清单里；② `任务卡/T6-*.md` 被「UI设计」回写第 11·12 节验收结果，而任务卡按 5.1 只有 PM 可写；③ `AI留痕/{T1,T2,T2b,T6,巡检-*}.md`、`测试素材交付.md` 等由测试/UI/代码负责写，但边界表只给「文档负责」（`D1-*`）与「PPT」（`D2-*`）留了 AI留痕 写权限，其余角色留痕目录未纳入 | 边界表文字缺口（文档漂移，**非越界行为**） | 按字面执行 5.1 的话，上述 3 类改动都会被误判成「越界」——而它们其实都有卡依据、也都记了台账。边界表与事实不符，会让巡检的 `git diff` 越界判据失真 | **2026-09-18 PM 已收编进 5.1**：① 测试负责可写目录补 `RuoYi-Vue-fast/pom.xml`（仅限 test-scope 依赖）+ `AI留痕/{T1,T2}-*` + `测试素材交付.md`；② UI设计补 `AI留痕/T6-*` + `AI留痕/截图/**`；③ 5.1 下方新增两条通用约定——「各角色可写自己卡号开头的 `AI留痕/` 文件，不改别人卡号」与「执行人可在自己认领的任务卡末尾追加验收结果小节、不改主体」。**本轮已把文字缺口补齐，未另开行为偏移** | ✅ 已收编 |
