...
> 「代码负责」**只剩一张未完成卡（T4 容器化交付）**，**T0、T3、T5 均已完成**；T4 可直接开工 —— `ci.yml` 里已备好注释着的 `docker` job，产出 Dockerfile 后取消注释即启用（同时销掉 **D-04 的 Dockerfile 侧**）。
