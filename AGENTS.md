...
> 「代码负责」现有两张未完成卡（**T4 容器化交付 / T5 设计模式**），**T0、T3 已完成**；建议顺序 **T5 优先**（它阻塞架构文档单项分最高的「设计模式 8 分」），T4 紧随（`ci.yml` 里已备好注释着的 docker job，产出 Dockerfile 后取消注释即启用）。
