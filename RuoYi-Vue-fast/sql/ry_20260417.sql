-- ============================================================================
-- [2/7] 若依基础表结构与基础数据
-- 用途：创建 sys_* / gen_* 全部若依基础表，并写入默认部门、岗位、用户、菜单、
--       字典、参数、公告等基础数据
-- 依赖：无（与 laboratory_schema.sql 之间没有外键依赖，两者顺序可互换；
--       本套脚本约定「实验室业务表在前、若依基础表在后」，读起来更顺）
-- 幂等：**否**。文件内部是 `drop table if exists` + `create table`，
--       重复执行会**清空 sys_* 表的数据**，请勿在已有数据的库上重跑。
--       （这也是它与其他 6 个脚本最大的不同：其余脚本都可重复执行）
-- 目标库：education_system
-- 说明：本文件是若依 RuoYi-Vue 3.9.2 的**官方初始化脚本，内容未做任何修改**，
--       仅由本项目补了以上这段顺序注释，便于按 [N/7] 顺序执行时辨认。
-- ============================================================================
