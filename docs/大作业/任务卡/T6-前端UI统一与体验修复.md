...
## 11. 2026-09-17 21:30 目视验收 + 截图交付（UI设计）｜ **本卡收口**

> 第 10.3 节列的 4 个「静态代码无法证伪」的不确定项，本轮**起了真实环境（MySQL + Redis + 后端 jar + 前端 dev server）
> 在浏览器里逐项量过**。同时采集了答辩 PPT 第 3 类素材截图。
> 完整记录见 `docs/大作业/AI留痕/T6-2026-09-17.md`。

### 11.1 第 10.3 节 4 项不确定性的结论

| # | 原判定 | 实测结论 | 状态 |
| :-: | --- | --- | :-: |
| **V-1** | 「本轮最大不确定性」：`overflow: hidden` 会不会破坏固定列 | 1366×768 横滚 411px 到底，固定列 `position: sticky` 保持，**单元格右边界 = 表头右边界 = 视口右缘（1345）**。**不破坏** | ✅ 解除 |
| **V-2** | 「卡里点名最容易漏的一处」，假设未验证 | 折叠后 `el-form` `display=none`、工具栏行 `border-top 1px` + 四角 `8px`。**属性选择器命中** | ✅ 解除 |
| **V-3** | 11 列在 1920 下是否还横滚 | 1920：容器 1678px = 列宽和 1678px，**无横滚**（1366 下需滚 411px，属预期） | ✅ 达成 |
| **V-4** | 220px 是否放得下 3 个按钮 | 真实库工单状态全为 3/4，`isManager` 下这类行**只出 1 个按钮**、无折行 → 3 按钮极端态当时未量 | ✅ 已补测（同日 22:5x，见 11.6） |

### 11.2 本轮新修（卡内）

**D-22：资产台账操作列 260px 放不下 4 个按钮，「删除」被挤到第二行**（行高被撑到 63px）。
已改 `asset/index.vue:63` 操作列 `width` **260 → 320**（4 个按钮实测需约 306px），复测：占 **1 行**、行高回落 **41px**、
1920 下仍无横滚。已按约定先登记 `AGENTS.md` 7.3 的 **D-22** 再动手。

**`element-ui.scss` 的注释修正**：原注释写「底部的确定/取消按钮**既看不到也滚不到**」—— 实测 **EP 2.13 的
`.el-overlay-dialog` 自带 `overflow: auto`，滚 327px 后按钮 bottom=752 < 768，是能滚到的**。已把注释改成经得起复测的版本
（`max-width` 真正处理的是**横向**溢出；门槛为视口 < 892px 才生效，实测 1920/1366/900 下弹窗仍是 860px，800→768px，600→568px）。

### 11.3 第 6 节验收命令的本轮实测输出

6 条 grep 全部符合期望（laboratory-page 7 处命中 / 3 个业务页引用 / 声明列宽合计 **1535px** / `allow-create` 零命中 /
三页均 `fixed="right"` / `element-ui.scss:58` 的 `calc(100vw - 32px)`）；
`vite build` → **`✓ 2545 modules transformed` / `✓ built in 43.30s` / EXIT=0**，无 error 无 warning；`dist/` 顶层结构正常、已清理。

**★ 一处口径必须记死**：「1535px」是 **SFC 里声明的列宽合计**。浏览器实测渲染值是 **1678px**（= 容器宽度），
因为 5 列用 `min-width`，`table-layout="fixed"` 会把剩余空间分给它们。**渲染值必然等于容器宽，与声明值不等是正常的**，
不代表「没收敛」。收敛判据是「声明值 1535 < 1920 屏内容区 1678 → 不横滚」。

### 11.4 截图交付（D2 卡第 3 类素材）

`docs/大作业/AI留痕/截图/T6-核心功能界面/`：`T6-01-首页运维看板` / `T6-02-资产台账` /
`T6-03-资产履历时间线` / `T6-04-报修处理时间线`，均 1920×1080、同一账号取景、无弹窗遮挡、可直接排版。
另有 5 张验收证据图（V-1/V-2/V-2b/V-4/报修列表/资产操作列）。
⚠️ **必须在 9-19 冻结点前用掉，之后再改 UI 这批截图即作废。**

### 11.5 本卡状态

**第 6 节验收命令 + 第 6 节「通过标准」的最后一条（浏览器目视）均已完成**，第 2 节复选框 15 项逐项对应到实测数据。
**T6 判定为完成**。11.1 的 V-4 极端态已于同日补测（见 11.6），**4 项不确定项现已全部收口，无遗留 ⚠️**。

**另新登记两个答辩级偏移（不属本卡范围，交对应角色）**：

- **D-20**：演示账号密码与全部文档不符（文档写 `123456`，实际哈希对应 `admin123`）。**按文档登录会直接失败。**
- **D-21**：真实库**缺 `laboratory:dashboard:view` 菜单**，非超管账号登录首页看不到看板（降级为占位提示）。
  → **本卡截图因此统一用 `admin` 取景**；根因是 `laboratory_permission_fix.sql` 未在真实库执行，交「代码负责」。

---

## 12. 2026-09-17 22:5x V-4 极端态补测（UI设计）｜ **最后一项不确定性收口**

### 12.1 为什么当时量不到

V-4 要验的是「操作列 **220px** 能否放下**同时出现的 3 个按钮**」。而真实库 4 条工单状态全是 `3 已完成` / `4 已拒绝`，
按 `canAudit` / `canStudentEdit` / `canEvaluate` / `canDelete` 四个条件，这些行**最多只出 1 个按钮**
（实测基线：3 行 `详情`、1 行 `详情/删除`，共 4 行）→ **3 按钮态在真实数据上不可达**。

### 12.2 补测方法：只在浏览器里注入合成行，**不写数据库**

环境当时还活着（前端 5173 / 后端 8080 / Chrome CDP 9222），于是写了一个探针
`.workbuddy/tools/ui_probe_v4.mjs`：

1. 用 CDP 打开 `/laboratory/repair`（admin 身份）；
2. 从表格行的 `__vueParentComponent` 沿 `parent` 链找到**页面组件**（判据：`setupState` 里有 `repairList`）；
3. 往 `repairList` 里 `push` 一行 `status='0'` 的合成数据 → 触发 `canAudit + canDelete` → **3 个按钮**；
4. 量完 `splice` 移除，并**复核合成行确实消失**。

> **为什么不用「往库里插一条待审核工单」**：那会改动答辩演示数据（工单数 4→5、还会把某条资产刷成「维修中」），
> 而 `AGENTS.md` 7.3 已登记的 **D-20 / D-21 说明演示数据状态本身就是答辩风险点**。
> **注入内存行是可逆的（刷新即消失）、零副作用，量到的是完全相同的渲染路径。**

### 12.3 实测结果

| 项 | 实测 | 判定 |
| --- | --- | --- |
| 按钮清单 | `详情(54px)` / `处理(54px)` / `删除(54px)` | 3 个，与卡第 5 节的推断一致 |
| 按钮纵向 `top` 去重 | **1 个值**（3 个按钮同一行） | **无换行** ✅ |
| 按钮总面积宽 | **186px** | 远小于 220px，**余量 34px** ✅ |
| 操作列单元格内容宽 | 220px | 未溢出 ✅ |
| 该行行高 | 55.4px | 正常（本表 `报修资产`/`申请人` 是 `.cell-stack` 双行，行高本就高于资产表） |
| 整表横滚（1920，含合成行） | 可视 1678 = 内容 1678 | **NO**，插入行未引入横滚 ✅ |

> 非管理员在 `status='0'` 时出的是 `详情 / 修改 / 删除`，与管理员那组**同为 3 个按钮、同为 2 个汉字**，
> 宽度等价，故只测一组即可代表极端态（按钮宽度由文案与图标决定，与角色无关）。

### 12.4 结论

**第 11.1 节的 4 项不确定项（V-1~V-4）现已全部收口，T6 无遗留 ⚠️。**
`220px` 这个数字由「推断最多 3 个按钮」升级为「实测 3 个按钮占 186px、单行不换行」——
**卡第 2.2 节要求写进代码注释的那个依据，现在是有实测数字支撑的了**（注释见 `repair/index.vue:86-88`）。

**顺带确认**：`320px` 给资产页那 4 个按钮（D-22，实测需约 306px）与此处的 `220px` 给 3 个按钮，
两处口径互不矛盾——**操作列宽度的算法是「同时最多可见按钮数」，不是「按钮总数」**。
